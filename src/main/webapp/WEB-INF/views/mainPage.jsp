...
<form class="search-bar mx-auto" action="/selectRoom" method="GET">
    <div class="search-option">
        <label>여행지</label>
        <input type="text" id="addressSearch" placeholder="여행지를 입력하세요" onkeyup="searchAddress()" />
        <div id="addressResults"></div> <!-- 검색 결과 표시할 div -->
    </div>
    <div class="search-option">
        <label>체크인</label>
...
<script type="text/javascript">
    ...
    function searchAddress() {
        const input = document.getElementById("addressSearch").value;
        ...
        if (input.length > 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    // 결과가 있을 경우, 결과를 표시
                    if (data.length > 0) {
                        data.forEach(function(accommodation) {
                            const div = document.createElement("div");
                            div.className = "result-item";
                            div.textContent = accommodation.address; // 주소로 결과 표시
                            div.onclick = function() {
                                document.getElementById("addressSearch").value = accommodation.address; // 선택한 주소로 입력
                                resultsContainer.innerHTML = ""; // 결과 컨테이너 비우기
                            };
